...
export default function SpecialOffer() {
  ...
  const blurbPhrases = [
    "Try this amazing #, you'll love it.*%",
    "Check out this #!",
    "You need this # in your life, immediately.*%",
    "Don't pass up the chance to get this #!",
  ];
        ...
        <div style={{ background: bgColor }} className="feat-daily-deal w-[70vw] h-[30vh] rounded-3xl flex items-center justify-end p-5 lg:pl-30 lg:pr-40">
          <div className="self-start mr-auto break-words w-100 h-[100%] flex flex-col">
            ...
            <Text c={textColor} className="text-1xl! mt-5! font-extrabold">
              {product.category !== "books" ? blurb.replace("#", product.name).replace("%", "") : blurb.replace("#", " book: " + product.name).replace("%", "")}
            </Text>
            <Button
              radius="lg"
              c={bgColor}
          ...
          <Image className="ml-auto w-60! invisible md:visible" radius={"lg"} h={"100%"} src={`/images/${product?.path}`}></Image>
        </div>
      </>
    );
}
